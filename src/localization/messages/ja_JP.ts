...
const ja_JP: Messages = {
  ...
  'onboarding.selectLanguageSubtitle': '言語を選択してください。' + '言語は後での設定変更ができます.',
  ...
  'onboarding.carousel.content.no-ip-tracking':
    '革新的なIPアドレスのない暗号化通信インフラを使用して、データの追跡やファイアウォールからブロックする心配がなく、どこからもCoNETネットワークにアクセスできます。',
  ...
  'platform.app.seguro.messenger.panels.addContact.firstMessage':
    'この連絡先にニックネームを設定することもできます。 カスタムの挨拶を送って、あなたが誰であるかを知らせましょう',
  ...
  'launchPage.FeatureArea1.2':
    'CONETブロックチェーンは、IP アドレスやメタデータを使用しない新しいネットワーク層をベースに、完全な匿名性、プライバシー、ユニバーサルアクセスなどをもっています',
  ...
  'platform.proxy.FeatureArea5.2':
    'CONETプロキシテクノロジは、VPNプライバシーの定義を書き換えます。 分散型で断片化された通信により、データはデータの海に完全に隠蔽されます。ウォレットアドレス通信ルーティングのネットワークにより、ユーザーは監視から消えます。',
  ...
  'platform.proxy.featureArea8Item.reload': '再ロード',
  ...
  'launchPage.FeatureArea3.2':
    'CONETはパーミッションレス、トラストレスのコミュニティ主導の相互作用を促進する、オープンソースプロジェクトです。ブロックチェーン技術で初めて匿名マイニングが実現しました。 ユーザーはCONETトークンを使用して、ガス料を支払い、報酬の獲得、トークンの委任、ノードの実行、ガバナンス計画への投票を行うことができます。',
  ...
  'launchPage.FeatureArea5.2':
    'サトシ・ナカモトはウォレット・アドレスでの資産転送を解決し、CONETはウォレット・アドレスでの高速かつ高スループットの情報交換を。CONETはWeb3ビジョンの実現に欠かせないネットワークインフラです',
  'launchPage.FeatureArea6.1': 'シンプルかつ軍事レベルの安全基準',
  'launchPage.FeatureArea6.2':
    'CONETプラットフォームは安全なサンドボックスをクライアントで作成し、サンドボックスに出入りするすべてのデータが暗号化されます。ストレージにはデーターを断片化され、暗号化した後で保存。メタデータのない断片は、万の一コンピュータに監視プログラムがあっても機能しない',
  'launchPage.FeatureArea7.1': 'dAPP開発は手軽に',
  'launchPage.FeatureArea7.2':
    'PWA技術で出来たCONETプラットフォームは、1つのコードで全デバイスに対応、dAPPの開発に低コストができ、分散型配布システムで、 ストアのレビューメカニズムを解決し、技術革新と投資を保証ができ',
  ...
  'platform.joinUS.miner.detail':
    '非信頼を前提に基づいて、ネットワークリソースプロバイダーのセキュリティを配慮した事がCONET設計の最優先事項です。セキュリティサンドボックス内の独立したスペースとIPアドレスのない独自のプライバシーマイナーは、CONETが他の製品の中でも際立っています。',
  ...
  'platform.miner.cloudnode.orderForm.detail':
    'CONET Layer Minusネットワークバックボーンノードは、CONETユーザーに高スループットのネットワークデータ転送を提供し、CNTPインセンティブにおける中断のない独占的な報酬を保証します。',
